...
parser.add_argument("--nclasses", help="number of classes",
...
parser.add_argument("--scale", type=float, default=1.0,
                    help="scaling factor, e.g. 0.0044")
parser.add_argument("--rgb", action="store_true",
                    help="whether to use RGB output, e.g. for torch pretrained models")
parser.add_argument("--confidences", type=str,
                    help="whether to output the confidence map, e.g. best", default='')
...
parameters_mllib = {'nclasses': nclasses,
                    'segmentation': True, 'gpu': True, 'gpuid': 0}
parameters_output = {}
try:
...
data = [args.image]
detect = dd.post_predict(sname, data, parameters_input,
...
if args.confidences:
    pixels_confs = np.array(
        detect['body']['predictions'][0]['confidences']['best'])
...
if not args.confidences:
    ...
    plt.imshow(np.reshape(
        pixels_confs, (imgsize['height'], imgsize['width'])), vmin=0, vmax=1)
plt.show()
